...
class OpenTargetsConnector(BaseConnector):
    ...
    async def fetch_associations(
        ...
        for row in rows:
            ...
            gene_symbol = target.get("approvedSymbol") or ""

            # Index by gene symbol
            if gene_symbol:
                gene_scores[gene_symbol] = round(score, 6)

            # Index by Swiss-Prot reviewed UniProt accession (if available)
            for protein_id in target.get("proteinIds", []):
                if protein_id.get("source") == "uniprot_swissprot":
                    uniprot = protein_id.get("id") or ""
                    if uniprot:
                        gene_scores[uniprot] = round(score, 6)
                    break  # only one Swiss-Prot accession per target
